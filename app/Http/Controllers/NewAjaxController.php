...
//                         ->where('person.company_id',$company_id)
//                         ->whereIn('person.state_id', $id)
//                         ->where('person.state_id','!=','0')
//                         ->where('person_status','=','1')
//                         ->pluck(DB::raw('CONCAT_WS(" ",first_name,middle_name,last_name) as name'), 'person.id');
//             $data['message'] = 'success';
//         } else {
//             $data['code'] = 401;
//             $data['result'] = '';
...
//     public function get_csa(Request $request)
//     {
//         if ($request->ajax()) {
//             $id = $request->id;
//             $data['code'] = 200;
...
//             $querydata = DB::table('csa');
//             if($id != 0){

//                 $querydata->where('csa_category_id', $id);
//             }
//             $query = $querydata->where('company_id',$company_id)
...
//             $class_arr = DB::table('_retailer_outlet_category')->where('company_id',$company_id)->pluck('outlet_category','id');
...
//                 $out[$user_id.$travel_date]['secondary_qty'] = DB::table($table_name)
...
//                 $out[$user_id.$travel_date]['secondary_sale'] = DB::table($table_name)
//                                     ->join('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
...
//                 $out[$user_id.$travel_date]['no_of_order_secondary'] = DB::table($table_name)
//                                     ->where('user_id',$t_value->user_id)
//                                     ->where('date',$t_value->travel_date)
//                                     ->where('company_id',$company_id)
//                                     ->distinct('order_id')->COUNT('order_id');
...
//         $table_name = TableReturn::table_return($from,$to);
...
//        $visit_count_data = DB::table($table_name)->join('location_view','location_view.l7_id','=',$table_name.'.location_id')->whereRaw("DATE_FORMAT(".$table_name.".date,'%Y-%m-%d') >='$from' and DATE_FORMAT(".$table_name.".date,'%Y-%m-%d') <='$to'")
//        ->where($table_name.'.company_id',$company_id)
//        ->groupBy('date','user_id');
//        if(!empty($state))
//         {
...
//        $visit_count = $visit_count_data->pluck(DB::raw("COUNT(DISTINCT ".$table_name.".retailer_id) as count"),DB::raw("CONCAT(user_id,date)"));


//        $productive_calls = DB::table($table_name)->where('call_status',1)->whereRaw("DATE_FORMAT(date,'%Y-%m-%d') >='$from' and DATE_FORMAT(date,'%Y-%m-%d') <='$to'")->where('company_id',$company_id)->groupBy('user_id','date')->pluck(DB::raw("COUNT(DISTINCT retailer_id) as productive_count"),DB::raw("CONCAT(user_id,date)"));


//        $other_data_data = DB::table($table_name)
//         ->join('location_view', 'location_view.l7_id', '=', $table_name.'.location_id')
//         ->join('dealer', 'dealer.id', '=', $table_name.'.dealer_id')
//         ->whereRaw("DATE_FORMAT(".$table_name.".date,'%Y-%m-%d') >='$from' and DATE_FORMAT(".$table_name.".date,'%Y-%m-%d') <='$to'")
//         ->where($table_name.'.company_id',$company_id)
//         ->where('dealer.company_id',$company_id)
//         ->select('user_id AS sale_user_id',$table_name.'.date as sale_date','location_id', 'location_view.l7_name', 'location_view.l6_name', $table_name.'.dealer_id', 'dealer.name as dealer_name','l7_id as l5_id',
//             DB::raw("(select COUNT(id) from retailer where location_id = ".$table_name.".location_id )as total_outlet"))
...
//     #date_wise_product_wise_report ends here
    ...
    public function dailyReportingReport(Request $request)
    {
        if ($request->ajax() ) {
            ...
            ->select('employee_info.contact_no as mobile','daily_reporting.working_with as working_with_id',DB::raw("CONCAT_WS(' ',f_name,l_name) AS user_name"),'employee_info.char_id as emp_code','designation.name as role_name',DB::raw("DATE_FORMAT(work_date,'%H:%i:%s') AS time"),DB::raw("DATE_FORMAT(work_date,'%d-%m-%Y') AS date"),'work_status','work_date','employee_info.user_primary_id AS user_id','remarks','attn_address')
            ...
            $user_records = $data1->groupBy('user_id','work_date','daily_reporting.id')->get();
            ...
            foreach ($user_records as $key => $value) {
                ...
                $user_record[$in]['l1_name']=!empty($value->l1_name)?$value->l1_name:'-';
                $user_record[$in]['l2_name']=!empty($value->l2_name)?$value->l2_name:'-';
                $user_record[$in]['l3_name']=!empty($value->l3_name)?$value->l3_name:'-';
                $user_record[$in]['l4_name']=!empty($value->l4_name)?$value->l4_name:'-';
                $user_record[$in]['l5_name']=!empty($value->l5_name)?$value->l5_name:'-';
                $user_record[$in]['l6_name']=!empty($value->l6_name)?$value->l6_name:'-';
                $user_record[$in]['emp_code']=$value->emp_code;
                $user_record[$in]['person_id_senior']=!empty($value->person_id_senior)?$value->person_id_senior:'-';
...
//                         ->where('user_sales_order_details.company_id', $company_id)
//                         ->select('user_sales_order_details.product_id','user_sales_order_details.quantity','user_sales_order_details.rate','catalog_product.name as product_name','user_sales_order_details.scheme_qty as weight','user_sales_order_details.order_id','weight_rate','weight_qty');
//                     }
//                     else{
//                     $proout = DB::table('user_sales_order_details')
//                             ->join('user_sales_order','user_sales_order.order_id','=','user_sales_order_details.order_id')
...
//                             ->where('user_sales_order_details.order_id', $uid)
//                             ->where('user_sales_order_details.company_id', $company_id)
//                             ->select('user_sales_order_details.product_id','user_sales_order_details.final_secondary_qty as quantity','user_sales_order_details.final_secondary_rate as rate','catalog_product.name as product_name','user_sales_order_details.scheme_qty as weight','user_sales_order_details.order_id','weight_rate','weight_qty');
//                     }
...
//                 $productName = DB::table('catalog_product')
//                                 ->where('company_id',$company_id)
//                                 ->pluck('name','id')->toArray();

//                 $productRate = DB::table('product_rate_list')
//                                 ->where('company_id',$company_id)
//                                 ->groupBy('product_id')
//                                 ->pluck('retailer_pcs_rate','product_id')->toArray();

//                $retailerStockDetailsQuery = DB::table('retailer_stock_details')
//                                 ->select('retailer_stock_details.product_id','retailer_stock_details.quantity','retailer_stock.user_id','retailer_stock.retailer_id','retailer_stock.date')
//                                 ->join('retailer_stock','retailer_stock.order_id','=','retailer_stock_details.order_id')
...
//                                 ->groupBy('retailer_stock_details.product_id','retailer_stock.user_id','retailer_stock.retailer_id','retailer_stock.date')
//                                 ->get();

//                 $detailsArrayMain = array();
//                 $finlaDetailsArrayMain = array();
//                 foreach ($retailerStockDetailsQuery as $rskey => $rsvalue) {
//                     $detailsArray['product_id'] = $rsvalue->product_id;
//                     $detailsArray['pr_name'] = !empty($productName[$rsvalue->product_id])?$productName[$rsvalue->product_id]:'';
//                     $detailsArray['rate'] = !empty($productRate[$rsvalue->product_id])?$productRate[$rsvalue->product_id]:'';
//                     $detailsArray['pieces'] = $rsvalue->quantity;

//                     $detailsArrayMain[$rsvalue->user_id.$rsvalue->retailer_id.$rsvalue->date][] = (object)$detailsArray;
//                     $finlaDetailsArrayMain[$rsvalue->user_id.$rsvalue->retailer_id.$rsvalue->date.$rsvalue->product_id] = $detailsArray;
//                 }


//                 // dd($finlaDetailsArrayMain);

//                 $finlaCatalog = DB::table('catalog_product')
//                                 ->where('company_id','=',$company_id)
//                                 ->pluck('name','id');
...
//             // $retailerStock = DB::table('retailer_stock')
//             //                     ->select('retailer_stock.user_id','retailer_stock.retailer_id','retailer_stock.date','retailer_stock.order_id')
...
//             //                     ->groupBy('retailer_stock.user_id','retailer_stock.retailer_id','retailer_stock.date')
//             //                     ->get();


//             // $retailerStockDetails = array();
//             // foreach ($retailerStock as $rskey => $rsvalue) {
//             //     $ruser_id = $rsvalue->user_id;
//             //     $rretailer_id = $rsvalue->retailer_id;
//             //     $rdate = $rsvalue->date;

//             //     $retailerStockDetails[$ruser_id.$rretailer_id.$rdate] = !empty($detailsArrayMain[$ruser_id.$rretailer_id.$rdate])?$detailsArrayMain[$ruser_id.$rretailer_id.$rdate]:array();


//             //     // $retailerStockDetails[$ruser_id.$rretailer_id.$rdate] = DB::table('retailer_stock_details')
//             //     //                 ->select(DB::raw("(quantity) as pieces"),'retailer_stock_details.product_id','catalog_product.name as pr_name','product_rate_list.retailer_pcs_rate as rate')
//             //     //                 ->join('retailer_stock','retailer_stock.order_id','=','retailer_stock_details.order_id')
...
//             //     //                 ->whereRaw("DATE_FORMAT(retailer_stock.date, '%Y-%m-%d') = '$rdate'")
//             //     //                 ->where('retailer_stock.company_id',$company_id)
//             //     //                 ->where('retailer_stock.user_id',$ruser_id)
//             //     //                 ->where('retailer_stock.retailer_id',$rretailer_id)
//             //     //                 ->where('retailer_stock_details.company_id',$company_id)
//             //     //                 ->groupBy('retailer_stock_details.product_id')
//             //     //                 ->get();

//             // }

//             // dd($retailerStockDetails);

//                 return view('reports.user_sale_report.Rajvaidyaajax', [
//                     'records' => $query,
//                     'details' => $out,
...
//                     'company_id'=> $company_id,
//                     // 'retailerStockDetails'=> $retailerStockDetails,
//                     'finlaCatalog'=> $finlaCatalog,
//                     'finlaDetailsArrayMain'=> $finlaDetailsArrayMain,
//                 ]);
...
//                         ->groupBy('location_id')
//                         ->groupBy('user_id')
//                         ->pluck(DB::raw('SUM(total_sale_value) as sale'),DB::raw("CONCAT(user_id,dealer_id,date,location_id) as concat_all"));



//         $main_query = Person::join($table_name,$table_name.'.user_id','=','person.id')
//                         ->leftJoin('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
//                         ->join('dealer','dealer.id','=',$table_name.'.dealer_id')
//                         ->join('_role','_role.role_id','=','person.role_id')
//                         ->leftJoin('retailer','retailer.id','=',$table_name.'.retailer_id')
//                         ->leftJoin('location_view','location_view.l7_id','=',$table_name.'.location_id')
//                         ->select('person.mobile as mobile','l3_name','l4_name','l5_name','l6_name','l7_name',$table_name.'.user_id as user_id','_role.rolename as rolename','emp_code as ecod','l7_name as name5','l7_id as location_5_id',DB::raw("SUM(rate*quantity) as total_sale"),'l3_name as name3','l6_name as name4','l4_id as zone_id','l2_name as name2',DB::raw("DATE_FORMAT(".$table_name.".date,'%Y-%m-%d') as dates"),DB::raw("DATE_FORMAT(".$table_name.".date,'%d-%m-%Y') as date"),$table_name.'.id AS uniq',$table_name.'.dealer_id AS dealer_id','person_id_senior',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as person_name"),'dealer.name AS dealer_name',$table_name.'.location_id AS usolid',$table_name.'.dealer_id AS did')
...
//                     $effetive_coverage_area_data = DB::table($table_name)->where('company_id',$company_id)->groupBy('user_id','dealer_id','date','location_id')->pluck(DB::raw("COUNT( DISTINCT retailer_id) as total"),DB::raw("CONCAT(user_id,dealer_id,date,location_id) as concat_all"));
//                     // end
...
//                     // end here  
//                     // for get_total_lpc 
//                     $get_total_lpc_data = DB::table('user_sales_order_details')->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')->where('user_sales_order_details.company_id',$company_id)->groupBy('user_id','dealer_id','date','location_id')->pluck(DB::raw("COUNT(product_id) as total"),DB::raw("CONCAT(user_id,dealer_id,date,location_id) as concat_all"));
//                     //  end here 

//                     // for get_total_sale_data  
//                     if(empty($check)){
//                     $get_total_sale_data = DB::table('user_sales_order_details')->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')->where('user_sales_order_details.company_id',$company_id)->groupBy('date','user_id','dealer_id','location_id')->pluck(DB::raw("SUM(rate*quantity) as total"),DB::raw("CONCAT(user_id,dealer_id,date,location_id) as concat_all"));
//                     }
//                     else{
...
//                     // for total call
//                     $total_call_data = DB::table($table_name)->where($table_name.'.company_id',$company_id)->groupBy('user_id','dealer_id','date','location_id')->pluck(DB::raw("COUNT(DISTINCT retailer_id) as totalCall"),DB::raw("CONCAT(user_id,dealer_id,date,location_id) as concat_all"));
//                     // end here 
...
//                                     ->where('retailer_status','=','1')
//                                     ->groupBy('location_id')
//                                     ->pluck(DB::raw("COUNT(DISTINCT retailer.id) as totalRetailer"),'location_id');


//                     // for total call
//                     $total_productive_call_data = DB::table($table_name)->where($table_name.'.company_id',$company_id)->where('call_status',1)->groupBy('user_id','dealer_id','date','location_id')->pluck(DB::raw("COUNT(DISTINCT retailer_id) as prCall"),DB::raw("CONCAT(user_id,dealer_id,date,location_id) as concat_all"));
//                     // end here 
...
//         $outType = DB::table('_retailer_outlet_type')
//                     ->where('company_id',$company_id)
//                     ->pluck('outlet_type','id')->toArray();

//         $main_query = DB::table($table_name)->join('person','person.id','=',$table_name.'.user_id')
//                     ->join('_role','_role.role_id','=','person.role_id')
//                     ->join('dealer','dealer.id','=',$table_name.'.dealer_id')
//                     ->join('retailer','retailer.id','=',$table_name.'.retailer_id')
//                     ->join('location_view','location_view.l7_id','=',$table_name.'.location_id')
//                     ->select('person.emp_code','rolename','person.mobile as mobile','l4_name','l5_name','l6_name','l7_name',$table_name.'.date as date',DB::raw("DATE_FORMAT(".$table_name.".date,'%d-%m-%Y') AS show_date"),$table_name.'.id AS uniq','location_view.l3_name as state','location_view.l6_name as city',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as person_name"),$table_name.'.user_id as user_id','dealer.name as dealer_name','person.person_id_senior as person_id_senior','retailer.name as retailer_name',$table_name.'.location_id as usolid',$table_name.'.dealer_id AS did',$table_name.'.retailer_id as retailer_id','retailer.outlet_type_id')
//                     ->whereRaw("DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')<='$to_date'")
//                     ->where($table_name.'.company_id',$company_id)
...
//                     if(empty($check)){
//                     $secondry_sale_data = DB::table('user_sales_order_details')->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")->where($table_name.'.company_id',$company_id)->groupBy('user_id','date','dealer_id','retailer_id')->pluck(DB::raw("SUM(rate*quantity) as total_sale_value"),DB::raw("CONCAT(user_id,date,dealer_id,retailer_id) as total"));
//                     }else{
...
//                         $senior_name[$user_id][$date] = !empty($senior_name_data[$person_id_senior])?$senior_name_data[$person_id_senior]:'';
...
//                      $scheme_sale_data = DB::table($table_name)
//                      ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
//                      ->where($table_name.'.company_id',$company_id)
...
//                             // ->join('person_login','person_login.person_id','=','person.id')
//                             // ->join('location_view','location_view.l7_id','=','retailer.location_id')
//                             ->select('purchase_order.invoice_no_p','invoice_date','reciept_image','dealer.other_numbers as dealer_no','dealer.id as dealer_id','fullfillment_order.time as time','fullfillment_order.order_id as order_id','fullfillment_order.date as date','location_3.name as l3_name','location_2.name as l2_name','dealer.name as dealer_name','purchase_order.sale_date as orderDate')
...
//         $uniqueTotalCall = DB::table($table_name)
//                         ->whereRaw("DATE_FORMAT(date, '%Y-%m-%d') >= '$from_date' AND DATE_FORMAT(date, '%Y-%m-%d') <= '$to_date'")
//                         ->where('company_id',$company_id)
//                         ->groupBy('user_id')
...
//         $uniqueProductiveCall = DB::table($table_name)
//                         ->join('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
//                         ->whereRaw("DATE_FORMAT(date, '%Y-%m-%d') >= '$from_date' AND DATE_FORMAT(date, '%Y-%m-%d') <= '$to_date'")
//                         ->groupBy('user_id')
//                         ->pluck(DB::raw("COUNT(DISTINCT retailer_id,date) as tc"),"user_id");
...
//         $masterRetailer = DB::table('retailer')
//                         ->where('company_id',$company_id)
//                         ->where('retailer_status','=','1')
//                         ->groupBy('retailer.id')
//                         ->pluck('outlet_type_id','retailer.id');

//         // dd($countRetailer);

//         // $countUserWiseRetailer = DB::table('dealer_location_rate_list')
//         //                 ->join('retailer','retailer.location_id','=','dealer_location_rate_list.location_id')
//         //                 ->where('retailer.company_id',$company_id)
//         //                 ->where('dealer_location_rate_list.company_id',$company_id)
//         //                 ->where('dealer_location_rate_list.user_id','!=','0')
//         //                 ->where('retailer_status','=','1')
//         //                 ->groupBy('user_id')
//         //                 ->pluck(DB::raw("COUNT(DISTINCT retailer.id) as count"),'user_id')->toArray();

//         $countUserWiseTypeWiseRetailer = DB::table('dealer_location_rate_list')
//                         ->join('retailer','retailer.location_id','=','dealer_location_rate_list.location_id')
//                         ->where('retailer.company_id',$company_id)
//                         ->where('dealer_location_rate_list.company_id',$company_id)
//                         ->where('dealer_location_rate_list.user_id','!=','0')
//                         ->where('retailer_status','=','1')
//                         ->groupBy('user_id','retailer.outlet_type_id')
//                         ->pluck(DB::raw("COUNT(DISTINCT retailer.id) as count"),DB::raw("CONCAT(user_id,retailer.outlet_type_id) as con"))->toArray();

//                         // dd($countUserWiseTypeWiseRetailer);


//           $countUserWiseTotalRetailer = DB::table('dealer_location_rate_list')
//                         ->select('user_id','retailer.id as retailer_id')
//                         ->join('retailer','retailer.location_id','=','dealer_location_rate_list.location_id')
//                         ->where('retailer.company_id',$company_id)
//                         ->where('dealer_location_rate_list.company_id',$company_id)
//                         ->where('dealer_location_rate_list.user_id','!=','0')
...
//           $finalTotalRetArray = array();
//         foreach ($countUserWiseTotalRetailer as $ftkey => $ftvalue) {
//             $finalTotalRetArray[$ftvalue->user_id][] = $ftvalue->retailer_id;
//         }


//         // dd($finalTotalRetArray);

//         $uniqueProductiveCallProductWise = DB::table($table_name)
//                         ->select('user_id','product_id','retailer_id')
//                         ->join('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
//                         ->whereRaw("DATE_FORMAT(date, '%Y-%m-%d') >= '$from_date' AND DATE_FORMAT(date, '%Y-%m-%d') <= '$to_date'")
//                         // ->where('user_id','=','1657')
//                         // ->where('product_id','=','4335')
//                         ->groupBy('user_id','product_id','retailer_id')
//                         ->get();


//         $finalProdWiseProductie = array();
//         foreach ($uniqueProductiveCallProductWise as $upcpkey => $upcpvalue) {
//             $finalProdWiseProductie[$upcpvalue->user_id.$upcpvalue->product_id][] = $upcpvalue->retailer_id;
//         }


//         // dd($finalProdWiseProductie);


//         $dsr =  DB::table($table_name)
//                    ->select(DB::raw("SUM(user_sales_order_details.quantity) as quantity"),DB::raw("CONCAT(user_id,product_id,final_product_type) as concat"),"final_product_type",DB::raw("COUNT(DISTINCT retailer_id,date) as uniqueProductiveCall"),DB::raw("SUM(user_sales_order_details.quantity*user_sales_order_details.rate) as sale_value"),"catalog_product.id as productId","catalog_product.quantity_per_case as quantity_per_case","catalog_product.quantiy_per_other_type as quantiy_per_other_type","catalog_product.final_product_type",$table_name.".user_id")
//                    ->join('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
//                    ->join('catalog_product','catalog_product.id','=','user_sales_order_details.product_id')
//                    ->whereRaw("DATE_FORMAT(date, '%Y-%m-%d') >= '$from_date' AND DATE_FORMAT(date, '%Y-%m-%d') <= '$to_date'")
...
//             $totalRetailer = !empty($finalTotalRetArray[$dsrvalue->user_id])?$finalTotalRetArray[$dsrvalue->user_id]:array();
//             $ProdRetailer = !empty($finalProdWiseProductie[$userProductConcat])?$finalProdWiseProductie[$userProductConcat]:array();

//             // $totalRetailer = !empty($finalTotalRetArray[1657])?$finalTotalRetArray[1657]:array();
//             // $ProdRetailer = !empty($finalProdWiseProductie[16574335])?$finalProdWiseProductie[16574335]:array();

//             // dd($ProdRetailer);

//             // if(!empty($ProdRetailer)){
//             //     foreach ($totalRetailer as $tkey => $tvalue) {
//             //         foreach ($ProdRetailer as $pkey => $pvalue) {
//             //             if($tvalue != $pvalue){
//             //                 $mainRet[$tvalue] = $tvalue;
//             //             }
//             //         }
//             //     }
//             // }else{
...
//             // $finalArrVal = array_values($mainRet);

//             $finalNonProductiveCalls[$userProductConcat] = array_diff($totalRetailer,$ProdRetailer);
//             // $finalNonProductiveCalls[$userProductConcat] = $finalArrVal;
...
//         foreach ($finalNonProductiveCalls as $fnkey => $fnvalue) {
//             foreach ($fnvalue as $fkey => $fvalue) {
//                 $cateKey = !empty($masterRetailer[$fvalue])?$masterRetailer[$fvalue]:'na';
                    
//                 $masterNonPro[$fnkey][$cateKey][] = $fvalue;
...
//         $retailerOutletType = DB::table('_retailer_outlet_type')
//                             ->where('company_id',$company_id)
//                             ->where('status','=','1')
...
//             'to_date' => $to_date,
//             'company_id' => $company_id,
//             'retailerOutletType' => $retailerOutletType,
//             // 'countRetailer' => $countRetailer,
//             // 'countUserWiseRetailer' => $countUserWiseRetailer,
//             'countUserWiseTypeWiseRetailer' => $countUserWiseTypeWiseRetailer,
//             'masterNonPro' => $masterNonPro,
...
//                     ->join('location_4','location_4.id','=','location_5.location_4_id')
//                     ->join('location_3','location_3.id','=','location_4.location_3_id')
//                     ->select('person.emp_code','rolename','person.mobile as mobile','location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name','location_7.name as l7_name',$table_name.'.date as date',DB::raw("DATE_FORMAT(".$table_name.".date,'%d-%m-%Y') AS show_date"),$table_name.'.id AS uniq',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as person_name"),$table_name.'.user_id as user_id','dealer.name as dealer_name','person.person_id_senior as person_id_senior',$table_name.'.location_id as usolid',$table_name.'.dealer_id AS did','location_7.id as location_id')
//                     ->whereRaw("DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')<='$to_date'")
//                     ->where($table_name.'.company_id',$company_id)
...
//                     {
//                         $dealer = $request->dealer;
//                         $main_query->whereIn($table_name.'.dealer_id', $dealer);
//                     }
//                     if (!empty($request->user)) 
//                     {
//                         $user = $request->user;
//                         $main_query->whereIn($table_name.'.user_id', $user);
//                     }
//                     if (!empty($request->role)) 
//                     {
//                         $role = $request->role;
...
//                         $senior_name[$user_id]['senior_name'] = !empty($senior_name_data[$person_id_senior])?$senior_name_data[$person_id_senior]:'';
//                         $senior_name[$user_id]['senior_role'] = !empty($senior_role_data[$person_id_senior])?$senior_role_data[$person_id_senior]:'';


//                          $senior_senior_name[$person_id_senior]['senior_name'] = !empty($senior_name_data[$seniorSeniorId])?$senior_name_data[$seniorSeniorId]:'';
//                         $senior_senior_name[$person_id_senior]['senior_role'] = !empty($senior_role_data[$seniorSeniorId])?$senior_role_data[$seniorSeniorId]:'';
...
//                     ->join('location_4','location_4.id','=','location_5.location_4_id')
//                     ->join('location_3','location_3.id','=','location_4.location_3_id')
//                     ->select('person.emp_code','rolename','person.mobile as mobile','location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name','location_7.name as l7_name',$table_name.'.date as date',DB::raw("DATE_FORMAT(".$table_name.".date,'%d-%m-%Y') AS show_date"),$table_name.'.id AS uniq',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as person_name"),$table_name.'.user_id as user_id','dealer.name as dealer_name','person.person_id_senior as person_id_senior',$table_name.'.location_id as usolid',$table_name.'.dealer_id AS did','location_7.id as location_id',DB::raw("COUNT(DISTINCT ".$table_name.".location_id,".$table_name.".date) as beat_visit"),'location_7.beat_frequency')
//                     ->whereRaw("DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')<='$to_date'")
//                     ->where($table_name.'.company_id',$company_id)
//                     ->groupBy('location_id')
...
//                                             ->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')
//                                             ->join('catalog_product','catalog_product.id','=','user_sales_order_details.product_id')
//                                             ->join('catalog_2','catalog_2.id','=','catalog_product.catalog_id')
//                                             ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
//                                             ->where($table_name.'.company_id',$company_id)->groupBy('location_id','catalog_2.id')
...
//         $lastLastMonth = date("Y-m", strtotime("-2 months"));
...
//          $currMonthSale = DB::table('user_sales_order_details')
//                             ->join('user_sales_order','user_sales_order.order_id','=','user_sales_order_details.order_id')
//                             ->whereRaw("DATE_FORMAT(date,'%Y-%m')='$currMonth'")
//                             ->where('user_sales_order.company_id',$company_id)
//                             ->where('user_sales_order_details.company_id',$company_id)
//                             ->groupBy('retailer_id')
//                             ->pluck(DB::raw("SUM(rate*quantity) as total_sale_value"),'retailer_id')->toArray();


//         $lastMonthSale = DB::table('user_sales_order_details')
//                             ->join('user_sales_order','user_sales_order.order_id','=','user_sales_order_details.order_id')
...
//         $lastLastMonthSale = DB::table('user_sales_order_details')
//                             ->join('user_sales_order','user_sales_order.order_id','=','user_sales_order_details.order_id')
//                             ->whereRaw("DATE_FORMAT(date,'%Y-%m')='$lastLastMonth'")
//                             ->where('user_sales_order.company_id',$company_id)
//                             ->where('user_sales_order_details.company_id',$company_id)
//                             ->groupBy('retailer_id')
//                             ->pluck(DB::raw("SUM(rate*quantity) as total_sale_value"),'retailer_id')->toArray();
...
//                     ->join('location_4','location_4.id','=','location_5.location_4_id')
//                     ->join('location_3','location_3.id','=','location_4.location_3_id')
//                     ->select('person.emp_code','rolename','person.mobile as mobile','location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name','location_7.name as l7_name',$table_name.'.date as date',DB::raw("DATE_FORMAT(".$table_name.".date,'%d-%m-%Y') AS show_date"),$table_name.'.id AS uniq',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as person_name"),$table_name.'.user_id as user_id','dealer.name as dealer_name','person.person_id_senior as person_id_senior',$table_name.'.location_id as usolid',$table_name.'.dealer_id AS did','location_7.id as location_id',DB::raw("COUNT(DISTINCT ".$table_name.".retailer_id,".$table_name.".date) as retailer_visit"),'retailer.id as retailer_id','retailer.name as retailer_name',DB::raw("GROUP_CONCAT(DISTINCT CONCAT_WS(' ',first_name,middle_name,last_name)) as finalUser"))
//                     ->whereRaw("DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')<='$to_date'")
//                     ->where($table_name.'.company_id',$company_id)
//                     ->groupBy('retailer_id')
//                     ->orderBy('location_3.name','ASC');
...
//                                             ->join('catalog_2','catalog_2.id','=','catalog_product.catalog_id')
//                                             ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
//                                             ->where($table_name.'.company_id',$company_id)->groupBy('retailer_id','catalog_2.id')
...
//                                 'currMonthSale'=>$currMonthSale,
//                                'lastMonthSale'=>$lastMonthSale,
//                                'lastLastMonthSale'=>$lastLastMonthSale,
...
//                     ->join('location_4','location_4.id','=','location_5.location_4_id')
//                     ->join('location_3','location_3.id','=','location_4.location_3_id')
//                     ->select('person.emp_code','rolename','person.mobile as mobile','location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name','location_7.name as l7_name',$table_name.'.date as date',DB::raw("DATE_FORMAT(".$table_name.".date,'%d-%m-%Y') AS show_date"),$table_name.'.id AS uniq',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as person_name"),$table_name.'.user_id as user_id','dealer.name as dealer_name','person.person_id_senior as person_id_senior',$table_name.'.location_id as usolid',$table_name.'.dealer_id AS did','location_7.id as location_id','retailer.id as retailer_id','retailer.name as retailer_name',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as user_name"),'person.mobile')
//                     ->whereRaw("DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(".$table_name.".date,'%Y-%m-%d')<='$to_date'")
//                     ->where($table_name.'.company_id',$company_id)
//                     ->groupBy('user_id')
//                     ->orderBy('location_3.name','ASC');
...
//                                             ->join('catalog_2','catalog_2.id','=','catalog_product.catalog_id')
//                                             ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
//                                             ->where($table_name.'.company_id',$company_id)->groupBy('user_id','catalog_2.id')
...
//      public function hitkaryAsmSaleReport(Request $request)
//     {
//         $company_id = Auth::user()->company_id;
//         $state = $request->state; 
//         $explodeDate = explode(" -", $request->date_range_picker);
...
//         $hitkaryRoles = array('527'); // ASM of Hitkary

//         $main_query = DB::table('person')
//                     ->join('person_login','person_login.person_id','=','person.id')
//                     ->join('_role','_role.role_id','=','person.role_id')
//                     ->join('location_6','location_6.id','=','person.town_id')
//                     ->join('location_5','location_5.id','=','location_6.location_5_id')
//                     ->join('location_4','location_4.id','=','location_5.location_4_id')
//                     ->join('location_3','location_3.id','=','person.state_id')
//                     ->select('person.emp_code','rolename','person.mobile as mobile','location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name','person.person_id_senior as person_id_senior',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as user_name"),'person.id as user_id')
//                     ->where('person.company_id',$company_id)
//                     ->whereIn('person.role_id',$hitkaryRoles)
//                     ->where('person_status','=','1')
//                     ->groupBy('person.id')
//                     ->orderBy('location_3.name','ASC');
//                     if (!empty($datasenior)) 
...
//                     $ASM_manager_name = DB::table('person')
//                         ->join('person_login','person_login.person_id','=','person.id')
//                         ->where('person_status','=','1')
//                         ->whereIn('person.role_id',$hitkaryRoles)
//                         ->pluck(DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as user_name"),'person.id');
//                     $final_ASM_managers = array();
//                     foreach ($ASM_manager_name as $mkey => $mvalue) {
//                         Session::forget('juniordata');
//                         $juniors_array=self::getJuniorUser($mkey);
//                         $juniors_array = $request->session()->get('juniordata');
//                         $final_ASM_managers[$mkey] = $juniors_array;
//                     }


//                     // dd($final_ASM_managers);
//                     if(empty($check)){
//                     $secondry_sale_data = DB::table('user_sales_order_details')
//                                             ->select(DB::raw("SUM(rate*quantity) as total_sale_value"),'user_id','catalog_2.id as c2_id',DB::raw("SUM(quantity) as quantity"))
//                                             ->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')
//                                             ->join('catalog_product','catalog_product.id','=','user_sales_order_details.product_id')
//                                             ->join('catalog_2','catalog_2.id','=','catalog_product.catalog_id')
//                                             ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
...
//                     }else{
//                     $secondry_sale_data = DB::table('user_sales_order_details')
//                                     ->select(DB::raw("ROUND(SUM(final_secondary_rate*final_secondary_qty),2) as total_sale_value"),'user_id','catalog_2.id as c2_id',DB::raw("SUM(final_secondary_qty) as quantity"))
//                                     ->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')
//                                     ->join('catalog_product','catalog_product.id','=','user_sales_order_details.product_id')
//                                     ->join('catalog_2','catalog_2.id','=','catalog_product.catalog_id')
//                                     ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
...
//                     $juniorArray = array();
//                     foreach ($secondry_sale_data as $skey => $svalue) {
                        
//                         $userid = $svalue->user_id;
//                         $catid = $svalue->c2_id;

//                         $juniorArray[$userid.$catid]['sale_value'] = $svalue->total_sale_value;
//                         $juniorArray[$userid.$catid]['quantity'] = $svalue->quantity;
...
//                     $finalASM = array();
//                     foreach ($final_ASM_managers as $asmkey => $asmvalue) {
//                         foreach ((array)$asmvalue as $jkey => $jvalue) {
//                             foreach ($category as $ckey => $cvalue) {
                                
//                                 $finalASM[$asmkey][$ckey]['sale_val'][] = !empty($juniorArray[$jvalue.$ckey]['sale_value'])?$juniorArray[$jvalue.$ckey]['sale_value']:'0';
//                                 $finalASM[$asmkey][$ckey]['qty'][] = !empty($juniorArray[$jvalue.$ckey]['quantity'])?$juniorArray[$jvalue.$ckey]['quantity']:'0';
...
//                     $userName = DB::table('person')
//                                 ->where('company_id',$company_id)
//                                 ->pluck(DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as user_name"),'person.id');
        


//                     return view('reports.hitkaryAsmSaleReport.ajax', [
//                                'secondry_sale'=>$secondry_sale_data,
//                                'main_query_data'=>$main_query_data,
//                                'company_id'=>$company_id,
//                                'category'=>$category,
...
//     public function salesManSecondaryHitkarySalesReport(Request $request)
//     {
//         $company_id = Auth::user()->company_id;
//         $state = $request->area;
//         $location2 = $request->location2;
...
//         //     $day_of_week = date('N', strtotime($dvalue));
//         //     if($day_of_week != '7'){
//         //     $workingDay[] = $dvalue;
//         //     }
//         // }
...
//         $allCallDetails = DB::table($table_name)
//                       ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND  DATE_FORMAT(date,'%Y-%m-%d')<='$to_date' ")
//                       ->where($table_name.'.company_id',$company_id)
//                       ->groupBy('user_id','date')
//                       ->pluck(DB::raw("COUNT(order_id) as orders"),DB::raw("CONCAT(user_id,date) as data"));
...
//                       ->where('user_sales_order_details.company_id',$company_id)
//                       ->groupBy('user_id','date')
//                       ->pluck(DB::raw("ROUND(SUM((rate*quantity)+(case_rate*user_sales_order_details.case_qty)),2) as sale"),DB::raw("CONCAT(user_id,date) as data"));

//         $callDetailsQuery = DB::table($table_name)
//                       ->select(DB::raw("MIN(time) as firstCall"),DB::raw("MAX(time) as lastCall"),DB::raw("CONCAT(user_id,date) as data"))
//                       ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND  DATE_FORMAT(date,'%Y-%m-%d')<='$to_date' ")
//                       ->where($table_name.'.company_id',$company_id)
//                       // ->where('call_status','=','1')
//                       ->groupBy('user_id','date')
...
//         $callDetails = array();
//         foreach ($callDetailsQuery as $ckey => $cvalue) {
//             $callDetails[$cvalue->data]['firstCall'] = $cvalue->firstCall;
//             $callDetails[$cvalue->data]['lastCall'] = $cvalue->lastCall;
//         }




//             return view('reports.salesManSecondaryHitkarySalesReport.ajax', [
//             'records' => $main_query,
//             'person' => $person,
//             'senior_name_data' => $senior_name_data,
//             'from_date'=> $from_date,
...
//     public function hitkaryDealerMonthSaleReport(Request $request)
//     {
//         $company_id = Auth::user()->company_id;
//         $state = $request->state; 
//         $explodeDate = explode(" -", $request->date_range_picker);
//         $from_date = date('Y-m-01',strtotime($request->from_month));
//         $to_date = date('Y-m-t',strtotime($request->to_month));
//         $check = DB::table('app_other_module_assign')->where('company_id',$company_id)->where('module_id',6)->first();

//         $startMonth = $request->from_month;
//         $endMonth = $request->to_month;
//         $date1 = date('Y-m-01',strtotime($startMonth));
//         $date2 = date('Y-m-t',strtotime($endMonth));
//         $table_name = TableReturn::table_return($date1,$date2);
...
//         $mainData = DB::table('dealer')
//                     ->select('dealer.id as dealer_id','dealer.name as dealer_name','location_3.name as state','location_6.name as town')
//                     ->join('location_6','location_6.id','=','dealer.town_id')
//                     ->join('location_5','location_5.id','=','location_6.location_5_id')
//                     ->join('location_4','location_4.id','=','location_5.location_4_id')
//                     ->join('location_3','location_3.id','=','location_4.location_3_id')
...
//                     if (!empty($datasenior)) 
//                     {
//                         $mainData->join('dealer_location_rate_list','dealer_location_rate_list.dealer_id','=','dealer.id')->whereIn('dealer_location_rate_list.user_id', $datasenior);
//                     }
//                     if(!empty($state))
//                     {
//                         $mainData->whereIn('location_3.id',$state);
//                     }
//                     if (!empty($request->location_3)) 
//                     {
//                         $location_3 = $request->location_3;
//                         $mainData->whereIn('location_3.id', $location_3);
//                     }
//                     if (!empty($request->location_4)) 
//                     {
//                         $location_4 = $request->location_4;
//                         $mainData->whereIn('location_4.id', $location_4);
//                     }
//                     if (!empty($request->location_5)) 
//                     {
//                         $location_5 = $request->location_5;
//                         $mainData->whereIn('location_5.id', $location_5);
//                     }
//                     if (!empty($request->location_6)) 
//                     {
//                         $location_6 = $request->location_6;
//                         $mainData->whereIn('location_6.id', $location_6);
//                     }
//                     if (!empty($request->dealer)) 
//                     {
//                         $dealer = $request->dealer;
//                         $mainData->whereIn('dealer.id', $dealer);
//                     }
//                     if (!empty($request->user)) 
//                     {
//                         $user = $request->user;
//                         $mainData->join('dealer_location_rate_list','dealer_location_rate_list.dealer_id','=','dealer.id')->whereIn('dealer_location_rate_list.user_id', $user);
//                     }
//                     if (!empty($request->role)) 
//                     {
//                         $role = $request->role;
//                         $mainData->whereIn('person.role_id', $role);
//                     }

//         $dealerData =     $mainData->groupBy('dealer.id')->get();
...
//                                             ->join('catalog_2','catalog_2.id','=','catalog_product.catalog_id')
//                                             ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
//                                             ->where($table_name.'.company_id',$company_id)->groupBy('dealer_id',DB::raw("DATE_FORMAT(date,'%Y-%m')"),'catalog_2.id')
//                                             ->pluck(DB::raw("SUM(rate*quantity) as total_sale_value"),DB::raw("CONCAT(dealer_id,DATE_FORMAT(date,'%Y-%m'),catalog_2.id) as total"));
//                     }else{
//                     $secondry_sale_data = DB::table('user_sales_order_details')
//                                     ->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')
//                                     ->join('catalog_product','catalog_product.id','=','user_sales_order_details.product_id')
...
//                                     ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
//                                     ->where($table_name.'.company_id',$company_id)->groupBy('dealer_id',DB::raw("DATE_FORMAT(date,'%Y-%m')"),'catalog_2.id')
//                                     ->pluck(DB::raw("ROUND(SUM(final_secondary_rate*final_secondary_qty),2) as total_sale_value"),DB::raw("CONCAT(dealer_id,DATE_FORMAT(date,'%Y-%m'),catalog_2.id) as total"));
//                     }
...
//                     return view('reports.hitkaryDealerMonthSaleReport.ajax', [
//                                'secondry_sale'=>$secondry_sale_data,
//                                'main_query_data'=>$dealerData,
//                                'company_id'=>$company_id,
//                                'category'=>$category,
//                                'monthArray'=>$monthArray,
//                             ]);
...
//      public function orderAgaintSuperStockistReport(Request $request)
//     {
//         if ($request->ajax() ) {
//             $company_id = Auth::user()->company_id;
//             $zone = $request->region;
...
//             $demandOrderDetails = DB::table('demand_order')
//                         ->select('csa.csa_name','order_date','total_value','remarks','demand_order.order_id')
//                         ->join('demand_order_details','demand_order_details.order_id','=','demand_order.order_id')
//                         ->join('dealer_person_login','dealer_person_login.dealer_id','=','demand_order.dealer_id')
//                         ->join('csa','csa.c_id','=','dealer_person_login.csa_id')
//                         ->join('location_3','location_3.id','=','csa.state_id')
//                         ->join('location_2','location_2.id','=','location_3.location_2_id')
//                         ->join('location_1','location_1.id','=','location_2.location_1_id')
//                         ->where('dealer_person_login.csa_id','!=','0')
//                         ->where('csa.company_id',$company_id)
...
//             {
//                 $location_3 = $request->location_3;
//                 $demandOrderDetails->whereIn('location_3.id', $location_3);
//             }
//             if (!empty($request->location_4)) 
//             {
//                 $location_4 = $request->location_4;
//                 $demandOrderDetails->whereIn('location_4.id', $location_4);
//             }
//             if (!empty($request->location_5)) 
//             {
//                 $location_5 = $request->location_5;
//                 $demandOrderDetails->whereIn('location_5.id', $location_5);
//             }
//             if (!empty($request->location_6)) 
//             {
//                 $location_6 = $request->location_6;
//                 $demandOrderDetails->whereIn('location_6.id', $location_6);
//             }

//             $demandOrder = $demandOrderDetails->groupBy('demand_order.order_id')->get();



//             $finalOut = array();
//             foreach ($demandOrder as $key => $value) {
               

//                 $user_record['csa_name']=$value->csa_name;
//                 $user_record['order_date']=$value->order_date;
//                 $user_record['total_value']=$value->total_value;
//                 $user_record['remarks']=$value->remarks;
//                 $user_record['order_id']=$value->order_id;
               

//                 $finalOut[] = $user_record;
//             }
//             // dd($finalOut);
//             return view('reports.orderAgaintSuperStockistReport.ajax', [
//                 'records' => $finalOut,
//                 'from_date' => $from_date,
//                 'to_date' => $to_date
//             ]);
...
//     public function orderAgaintSuperStockistReportReplica(Request $request)
//     {
//         if ($request->ajax() ) {
//             $company_id = Auth::user()->company_id;
//             $zone = $request->region;
...
//             }
//             // dd($finalOut);
//             return view('reports.orderAgaintSuperStockistReportReplica.ajax', [
//                 'records' => $finalOut,
//                 'from_date' => $from_date,
//                 'to_date' => $to_date
//             ]);
...
//     public function dailyEvaluationReport(Request $request)
//     {
//         if ($request->ajax() ) {
//             $company_id = Auth::user()->company_id;
//             $zone = $request->region;
...
//             ->join('location_2','location_2.id','=','location_3.location_2_id')
//             ->join('location_1','location_1.id','=','location_2.location_1_id')
//             ->select('person_id_senior','person.mobile',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) AS user_name"),'location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name','location_1.name as l1_name','location_2.name as l2_name','emp_code','rolename AS role_name','person.id AS user_id')
//             ->where('person.company_id',$company_id)
//             ->where('location_1.company_id',$company_id)
//             ->where('location_2.company_id',$company_id)
//             ->where('location_3.company_id',$company_id)
...
//                 $data1->whereIn('user_id', $user);
//             }
//             $user_records = $data1->where('person_status',1)->groupBy('person.id')->orderBy('l3_name','ASC')->get();

//             // dd($user_records);

//             $tourPlan = DB::table('monthly_tour_program')
//                         ->where('company_id',$company_id)
...
//                         ->groupBy('person_id')
//                         ->pluck('town','person_id');



//             $actualTown = DB::table($table_name)
//                         ->join('location_7','location_7.id','=',$table_name.'.location_id')
//                         ->join('location_6','location_6.id','=','location_7.location_6_id')
//                         ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT(date,'%Y-%m-%d') <='$to_date'")
//                         ->where('location_7.company_id',$company_id)
//                         ->where('location_6.company_id',$company_id)
//                         ->groupBy('user_id')
//                         ->pluck(DB::raw("GROUP_CONCAT(DISTINCT location_6.name) as workingTown"),'user_id');
...
//                                 ->where($table_name.'.company_id',$company_id)
//                                 ->groupBy('user_id')
//                                 ->pluck(DB::raw("COUNT(DISTINCT retailer_id) as totalCall"),'user_id');


//             if(empty($check)){
//                 $productiveData =  DB::table($table_name)
//                                 ->select(DB::raw("GROUP_CONCAT(DISTINCT dealer.name) as dealerName"),DB::raw("GROUP_CONCAT(DISTINCT retailer.name) as retailerName"),DB::raw("COUNT(DISTINCT retailer_id) as productiveCall"),DB::raw("ROUND(SUM((rate*quantity)+(case_rate*user_sales_order_details.case_qty)),2) as sale"),'user_id')
//                                 ->join('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
//                                 ->join('dealer','dealer.id','=',$table_name.'.dealer_id')
//                                 ->join('retailer','retailer.id','=',$table_name.'.retailer_id')
//                                 ->whereRaw("DATE_FORMAT($table_name.date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT($table_name.date,'%Y-%m-%d') <='$to_date'")
...
//             }else{
//                  $productiveData =  DB::table($table_name)
//                                 ->select(DB::raw("GROUP_CONCAT(DISTINCT dealer.name) as dealerName"),DB::raw("GROUP_CONCAT(DISTINCT retailer.name) as retailerName"),DB::raw("COUNT(DISTINCT retailer_id) as productiveCall"),DB::raw("ROUND(SUM(final_secondary_rate*final_secondary_qty),2) as sale"),'user_id')
//                                 ->join('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
//                                 ->join('dealer','dealer.id','=',$table_name.'.dealer_id')
//                                 ->join('retailer','retailer.id','=',$table_name.'.retailer_id')
//                                 ->whereRaw("DATE_FORMAT($table_name.date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT($table_name.date,'%Y-%m-%d') <='$to_date'")
...
//             $finalProdData = array();
//             foreach ($productiveData as $key => $value) {
//                 $finalProdData[$value->user_id]['dealerName'] = $value->dealerName;
//                 $finalProdData[$value->user_id]['retailerName'] = $value->retailerName;
//                 $finalProdData[$value->user_id]['productiveCall'] = $value->productiveCall;
//                 $finalProdData[$value->user_id]['sale'] = $value->sale;
//             }


//             // if(empty($check)){
//             // $primaryData = DB::table('user_primary_sales_order')
//             //                 ->select('created_person_id',DB::raw("((rate*quantity)+(cases*pr_rate)) as amount"),DB::raw("((quantity/catalog_product.quantity_per_case)+(cases)) as cases"))
//             //                 ->join('user_primary_sales_order_details','user_primary_sales_order_details.order_id','=','user_primary_sales_order.order_id')
//             //                 ->join('catalog_product','catalog_product.id','=','user_primary_sales_order_details.product_id')
//             //                 ->whereRaw("DATE_FORMAT(sale_date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT(sale_date,'%Y-%m-%d') <='$to_date'")
//             //                 ->where('user_primary_sales_order.company_id',$company_id)
...
//             // }else{
//             //     $primaryData = DB::table('user_primary_sales_order')
//             //                 ->select('created_person_id',DB::raw("(final_secondary_qty*final_secondary_rate) as amount"),"final_secondary_rate as cases")
//             //                 ->join('user_primary_sales_order_details','user_primary_sales_order_details.order_id','=','user_primary_sales_order.order_id')
//             //                 ->join('catalog_product','catalog_product.id','=','user_primary_sales_order_details.product_id')
//             //                 ->whereRaw("DATE_FORMAT(sale_date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT(sale_date,'%Y-%m-%d') <='$to_date'")
//             //                   ->where('user_primary_sales_order.company_id',$company_id)
...
//             // $finalPrimaryData = array();
//             // foreach ($primaryData as $pkey => $pvalue) {
//             //     $finalPrimaryData[$pvalue->created_person_id]['cases'] = $pvalue->cases;
//             //     $finalPrimaryData[$pvalue->created_person_id]['amount'] = $pvalue->amount;
//             // }


//             $towns = DB::table('location_6')
//                     ->where('company_id',$company_id)
//                     ->pluck('name','id');

//               if(empty($check)){
//                 $retailerWiseData =  DB::table($table_name)
//                                  ->select('dealer.name as dealerName','retailer.name as retailerName',DB::raw("COUNT(DISTINCT retailer_id) as productiveCall"),DB::raw("ROUND(SUM((rate*quantity)+(case_rate*user_sales_order_details.case_qty)),2) as salesec"),'user_id','retailer.landline','retailer.other_numbers')
//                                 ->join('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
//                                 ->join('dealer','dealer.id','=',$table_name.'.dealer_id')
//                                 ->join('retailer','retailer.id','=',$table_name.'.retailer_id')
//                                 ->whereRaw("DATE_FORMAT($table_name.date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT($table_name.date,'%Y-%m-%d') <='$to_date'")
...
//                                 ->where('dealer.company_id',$company_id)
//                                 ->where('retailer.company_id',$company_id)
//                                 ->groupBy('user_id','retailer_id')
//                                 ->get();

//             }else{
//                  $retailerWiseData =  DB::table($table_name)
//                                 ->select('dealer.name as dealerName','retailer.name as retailerName',DB::raw("COUNT(DISTINCT retailer_id) as productiveCall"),DB::raw("ROUND(SUM(final_secondary_rate*final_secondary_qty),2) as salesec"),'user_id','retailer.landline','retailer.other_numbers')
//                                 ->join('user_sales_order_details','user_sales_order_details.order_id','=',$table_name.'.order_id')
//                                 ->join('dealer','dealer.id','=',$table_name.'.dealer_id')
//                                 ->join('retailer','retailer.id','=',$table_name.'.retailer_id')
//                                 ->whereRaw("DATE_FORMAT($table_name.date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT($table_name.date,'%Y-%m-%d') <='$to_date'")
...
//             $finalRetailerData = array();
//             foreach ($retailerWiseData as $rwkey => $rwvalue) {
//                 $retailerData['dealerName'] = $rwvalue->dealerName;
//                 $retailerData['retailerName'] = $rwvalue->retailerName;
//                 $retailerData['retailerContact'] = !empty($rwvalue->landline)?$rwvalue->landline:$rwvalue->other_numbers;
//                 $retailerData['productiveCall'] = $rwvalue->productiveCall;
//                 $retailerData['salesec'] = $rwvalue->salesec;

//                 $finalRetailerData[$rwvalue->user_id][] = $retailerData;
//             }

//             // dd($finalRetailerData);
...
//                 $user_record[$user_id]['l2_name']=$value->l2_name;
//                 $user_record[$user_id]['l3_name']=$value->l3_name;
//                 $user_record[$user_id]['l4_name']=$value->l4_name;
//                 $user_record[$user_id]['l5_name']=$value->l5_name;
//                 $user_record[$user_id]['l6_name']=$value->l6_name;
//                 $user_record[$user_id]['emp_code']=$value->emp_code;
//                 $user_record[$user_id]['person_id_senior']=$value->person_id_senior;
//                 $user_record[$user_id]['mobile']=$value->mobile;
//                 $user_record[$user_id]['role_name']=$value->role_name;

//                 $town_id = !empty($tourPlan[$user_id])?$tourPlan[$user_id]:'0';

//                 $user_record[$user_id]['tourPlanStation']= !empty($towns[$town_id])?$towns[$town_id]:'';
//                 $user_record[$user_id]['actualPlanStation']= !empty($actualTown[$user_id])?$actualTown[$user_id]:'';

//                 $user_record[$user_id]['totalCall']= !empty($totalCallData[$user_id])?$totalCallData[$user_id]:'';


//                 $user_record[$user_id]['dealerName']= !empty($finalProdData[$user_id]['dealerName'])?$finalProdData[$user_id]['dealerName']:'';
//                 $user_record[$user_id]['retailerName']= !empty($finalProdData[$user_id]['retailerName'])?$finalProdData[$user_id]['retailerName']:'';
//                 $user_record[$user_id]['productiveCall']= !empty($finalProdData[$user_id]['productiveCall'])?$finalProdData[$user_id]['productiveCall']:'';
//                 $user_record[$user_id]['sale']= !empty($finalProdData[$user_id]['sale'])?$finalProdData[$user_id]['sale']:'';


//                 // $user_record[$user_id]['cases']= !empty($finalPrimaryData[$user_id]['cases'])?$finalPrimaryData[$user_id]['cases']:'';
//                 // $user_record[$user_id]['primarysale']= !empty($finalPrimaryData[$user_id]['amount'])?$finalPrimaryData[$user_id]['amount']:'';


//                 $user_record[$user_id]['retailerArray']= !empty($finalRetailerData[$user_id])?$finalRetailerData[$user_id]:array();

//             }

//             // dd($user_record);
//             return view('reports.dailyEvaluationReport.ajax', [
//                 'records' => $user_record,
//                 'from_date' => $from_date,
//                 'to_date' => $to_date
//             ]);
...
//     public function hitkaryRetailerWiseEvaluationReport(Request $request)
//     {
//         $company_id = Auth::user()->company_id;
//         $state = $request->state; 
//         // $explodeDate = explode(" -", $request->date_range_picker);
...
//         $main_query = DB::table('retailer')
//                     ->join('location_7','location_7.id','=','retailer.location_id')
//                     ->join('location_6','location_6.id','=','location_7.location_6_id')
//                     ->join('location_5','location_5.id','=','location_6.location_5_id')
//                     ->join('location_4','location_4.id','=','location_5.location_4_id')
//                     ->join('location_3','location_3.id','=','location_4.location_3_id')
//                      ->select('location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name','location_7.name as l7_name','retailer.id as retailer_id','retailer.name as retailer_name')
//                     ->where('retailer.company_id',$company_id)
//                     ->where('retailer_status','=','1')
//                     ->groupBy('retailer.id')
//                     ->orderBy('location_3.name','ASC');
...
//                     if (!empty($datasenior)) 
//                     {
//                         $main_query->whereIn('retailer.created_by_person_id', $datasenior);
//                     }
//                     if(!empty($state))
//                     {
//                         $main_query->whereIn('location_3.id',$state);
...
//                     {
//                         $dealer = $request->dealer;
//                         $main_query->whereIn('retailer.dealer_id', $dealer);
//                     }
//                     if (!empty($request->user)) 
//                     {
//                         $user = $request->user;
//                         $main_query->whereIn('retailer.created_by_person_id', $user);
//                     }
//                     if (!empty($request->role)) 
//                     {
//                         $role = $request->role;
...
//                     if(empty($check)){
//                     $secondry_sale_month_data = DB::table('user_sales_order_details')
//                                             ->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')
//                                             ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$date1' AND DATE_FORMAT(date,'%Y-%m-%d')<='$date2'")
//                                             ->where($table_name.'.company_id',$company_id)
//                                             ->where('user_sales_order_details.company_id',$company_id)
//                                             ->groupBy('retailer_id',DB::raw("DATE_FORMAT(date,'%Y-%m')"))
//                                             ->pluck(DB::raw("SUM(rate*quantity) as total_sale_value"),DB::raw("CONCAT(retailer_id,DATE_FORMAT(date,'%Y-%m')) as total"));
//                     }else{
//                     $secondry_sale_month_data = DB::table('user_sales_order_details')
//                                     ->join($table_name,$table_name.'.order_id','=','user_sales_order_details.order_id')
//                                     ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$date1' AND DATE_FORMAT(date,'%Y-%m-%d')<='$date2'")
...
//                                     ->where('user_sales_order_details.company_id',$company_id)
//                                     ->groupBy('retailer_id',DB::raw("DATE_FORMAT(date,'%Y-%m')"))
//                                     ->pluck(DB::raw("ROUND(SUM(final_secondary_rate*final_secondary_qty),2) as total_sale_value"),DB::raw("CONCAT(retailer_id,DATE_FORMAT(date,'%Y-%m')) as total"));
//                     }
...
//                                             ->pluck(DB::raw("SUM(rate*quantity) as total_sale_value"),DB::raw("CONCAT(retailer_id,catalog_2.id) as total"))->toArray();
...
//                                     ->where('catalog_2.company_id',$company_id)
//                                     ->groupBy('retailer_id','catalog_2.id')
//                                     ->pluck(DB::raw("ROUND(SUM(final_secondary_rate*final_secondary_qty),2) as total_sale_value"),DB::raw("CONCAT(retailer_id,catalog_2.id) as total"))->toArray();
//                     }



//                       $retailerVisit = DB::table($table_name)
//                                             ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$date1' AND DATE_FORMAT(date,'%Y-%m-%d')<='$date2'")
//                                             ->where($table_name.'.company_id',$company_id)
//                                             ->groupBy('retailer_id')
//                                             ->pluck(DB::raw("COUNT(DISTINCT retailer_id,date) as total_sale_value"),'retailer_id')->toArray();
...
//                     $userNames = DB::table('dealer_location_rate_list')
//                                 ->join('retailer','retailer.location_id','=','dealer_location_rate_list.location_id')
//                                 ->join('person','person.id','=','dealer_location_rate_list.user_id')
//                                 ->where('dealer_location_rate_list.company_id',$company_id)
//                                 ->where('retailer.company_id',$company_id)
//                                 ->where('person.company_id',$company_id)
//                                 ->groupBy('retailer.id')
//                                 ->pluck(DB::raw("GROUP_CONCAT(DISTINCT CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name)) as concat"),'retailer.id')->toArray();

//                     return view('reports.hitkaryRetailerWiseEvaluationReport.ajax', [
//                                'secondry_sale'=>$secondry_sale_data,
//                                'main_query_data'=>$main_query_data,
//                                'company_id'=>$company_id,
//                                'category'=>$category,
//                                'monthArray'=>$monthArray,
//                                'secondry_sale_month_data'=>$secondry_sale_month_data,
//                                'retailerVisit'=>$retailerVisit,
//                                'userNames'=>$userNames,
...
//     public function assigningDetailReport(Request $request)
//     {
//         if ($request->ajax() ) {
//             $company_id = Auth::user()->company_id;
//             $zone = $request->region;
...
//                             ->where('retailer_status','=','1')
//                             ->groupBy('location_id')
//                             ->pluck(DB::raw("COUNT(retailer.id) as retailerCount"),'location_id')->toArray();


//              $data1 = DB::table('person')
//                     ->select('person.id as user_id','dealer_location_rate_list.dealer_id','dealer_location_rate_list.location_id','location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as user_name"),'person.person_id_senior','_role.rolename as role_name','person.emp_code','person.mobile','dealer.name as dealer_name','location_7.name as l7_name','location_7.id as l7_id')
//                     ->join('dealer_location_rate_list','dealer_location_rate_list.user_id','=','person.id')
//                     ->join('dealer','dealer.id','=','dealer_location_rate_list.dealer_id')
//                     ->join('person_login','person_login.person_id','=','person.id')
//                     ->join('_role','_role.role_id','=','person.role_id')
...
//                 $data1->whereIn('user_id', $user);
//             }
//             $user_records = $data1->where('person_status',1)->groupBy('person.id','dealer_location_rate_list.dealer_id','dealer_location_rate_list.location_id')
...
//             // dd($user_record);
//             return view('reports.assigningDetailReport.ajax', [
...
//       public function dailyReportingReportBtw(Request $request)
//     {
//         if ($request->ajax() ) {
//             $company_id = Auth::user()->company_id;
//             $zone = $request->region;
...
//             $attendanceQuery = DB::table('user_daily_attendance')
//                                 ->select(DB::raw("DATE_FORMAT(work_date,'%Y-%m-%d') as work_date"),'user_id','track_addrs',DB::raw("DATE_FORMAT(work_date,'%H:%i:%s') as work_time"))
//                                 ->where('company_id',$company_id)
//                                 ->whereRaw("DATE_FORMAT(user_daily_attendance.work_date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT(user_daily_attendance.work_date,'%Y-%m-%d') <='$to_date'")
//                                 ->groupBy('work_date','user_id')
//                                 ->get()->toArray();
...
//             foreach ($attendanceQuery as $akey => $avalue) {
//                 $oak['work_date'] = $avalue->work_date;
//                 $oak['user_id'] = $avalue->user_id;
//                 $oak['track_addrs'] = $avalue->track_addrs;
//                 $oak['work_time'] = $avalue->work_time;

//                 $attendanceData[$avalue->user_id.$avalue->work_date] = $oak;
//             }



//             $checkoutQuery = DB::table('check_out')
//                                 ->select(DB::raw("DATE_FORMAT(work_date,'%Y-%m-%d') as work_date"),'user_id','attn_address as track_addrs',DB::raw("DATE_FORMAT(work_date,'%H:%i:%s') as work_time"))
//                                 ->where('company_id',$company_id)
//                                 ->whereRaw("DATE_FORMAT(check_out.work_date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT(check_out.work_date,'%Y-%m-%d') <='$to_date'")
//                                 ->groupBy('work_date','user_id')
//                                 ->get()->toArray();
...
//             $checkoutData = array();
//             foreach ($checkoutQuery as $ckey => $cvalue) {
//                 $ock['work_date'] = $cvalue->work_date;
//                 $ock['user_id'] = $cvalue->user_id;
//                 $ock['track_addrs'] = $cvalue->track_addrs;
//                 $ock['work_time'] = $cvalue->work_time;

//                 $checkoutData[$cvalue->user_id.$cvalue->work_date] = $ock;
//             }



//              $dailyReportQuery = DB::table('daily_reporting')
//                                 ->select(DB::raw("DATE_FORMAT(work_date,'%Y-%m-%d') as work_date"),'user_id','attn_address as track_addrs',DB::raw("DATE_FORMAT(work_date,'%H:%i:%s') as work_time"))
//                                 ->where('company_id',$company_id)
//                                 ->whereRaw("DATE_FORMAT(daily_reporting.work_date,'%Y-%m-%d') >='$from_date' and DATE_FORMAT(daily_reporting.work_date,'%Y-%m-%d') <='$to_date'")
//                                 ->groupBy('daily_reporting.id')
//                                 ->orderBy('work_time','ASC')
//                                 ->get()->toArray();

//             $dailyReportData = array();
//             foreach ($dailyReportQuery as $dkey => $dvalue) {
//                 $odk['work_date'] = $dvalue->work_date;
//                 $odk['user_id'] = $dvalue->user_id;
//                 $odk['track_addrs'] = $dvalue->track_addrs;
//                 $odk['work_time'] = $dvalue->work_time;

//                 $dailyReportData[$dvalue->user_id.$dvalue->work_date][] = $odk;
//             }
...
//             ->join('location_4','location_4.id','=','location_5.location_4_id')
//             ->join('location_3','location_3.id','=','person.state_id')
//             ->select('person_id_senior','person.mobile',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) AS user_name"),'location_3.name as l3_name','location_4.name as l4_name','location_5.name as l5_name','location_6.name as l6_name','emp_code','rolename AS role_name','person.id AS user_id','remarks',DB::raw("DATE_FORMAT(user_daily_attendance.work_date,'%Y-%m-%d') as work_date"))
//             ->where('user_daily_attendance.company_id',$company_id)
//             ->where('person.company_id',$company_id)
//             ->where('location_3.company_id',$company_id)
//             ->where('location_4.company_id',$company_id)
...
//                 $data1->whereIn('user_id', $user);
//             }
//             $user_records = $data1->where('person_status',1)->groupBy('user_id','work_date')->get();
...
//                 $user_record[$in]['mobile']=$value->mobile;
//                 $user_record[$in]['role_name']=$value->role_name;
//                 $user_record[$in]['work_date']=$value->work_date;

//                 $user_record[$in]['attLocation']= !empty($attendanceData[$in]['track_addrs'])?$attendanceData[$in]['track_addrs']:'';
//                 $user_record[$in]['attTime']= !empty($attendanceData[$in]['work_time'])?$attendanceData[$in]['work_time']:'';

//                 $user_record[$in]['chkLocation']= !empty($checkoutData[$in]['track_addrs'])?$checkoutData[$in]['track_addrs']:'';
//                 $user_record[$in]['chkTime']= !empty($checkoutData[$in]['work_time'])?$checkoutData[$in]['work_time']:'';


//                 $user_record[$in]['dailyReportCount']= !empty($dailyReportData[$in])?COUNT($dailyReportData[$in]):'0';
//                 $user_record[$in]['dailyReportArray']= !empty($dailyReportData[$in])?$dailyReportData[$in]:array();

//                 $maxLoop[] = !empty($dailyReportData[$in])?COUNT($dailyReportData[$in]):'0';

              
//             }

//             $maxCount = max($maxLoop);
//             return view('reports.user-daily-reporting.btwAjax', [
//                 'records' => $user_record,
//                 'from_date' => $from_date,
//                 'to_date' => $to_date,
//                 'maxCount' => $maxCount,
...
//     public function userDistanceReport(Request $request)
//     {
//         $company_id = Auth::user()->company_id;
//         $state = $request->area;
//         $location2 = $request->location2;
...
//                 {
//                     $person_data->whereIn('person.department_id',$request->department);
//                 }
//                  if(!empty($request->role))
//                 {
//                     $person_data->whereIn('person.role_id',$request->role);
...
//         $first_call = DB::table('user_distance_tracking')
//                       ->where('company_id',$company_id)  
//                       ->whereRaw("DATE_FORMAT(user_distance_tracking.track_date,'%Y-%m-%d')>='$from_date' AND  DATE_FORMAT(user_distance_tracking.track_date,'%Y-%m-%d')<='$to_date' ")
//                       ->groupBy('user_id','track_date')
//                       ->pluck('total_km',DB::raw("CONCAT(user_id,track_date) as data"));  
...
//         return view('reports.userDistanceReport.ajax', [
//             'records' => $person,
//             'senior_name_data' => $senior_name_data,
//             'from_date'=> $from_date,
//             'to_date'=> $to_date,
...
//     public function dms_upload_documents_transport(Request $request)
//     {
//         // dd($request);
//         $company_id = Auth::user()->company_id;
//         if ($request->hasFile('imageFile')) {
...
//                         $imageName = $custom_image_name . '.' . $file->getClientOriginalExtension();
//                         $file_name[] = $imageName;
//                         $destinationPath = public_path('/transport_document/');
//                         $file->move($destinationPath , $imageName);
//                         $delete_data = DB::table('dms_transport_document_details')
//                                     ->where('transport_id',$request->dms_transport_id)
//                                     ->where('document_id',!empty($request->document_id[$file_key])?$request->document_id[$file_key]:'0')
//                                     ->delete();

//                         $personImage = DB::table('dms_transport_document_details')->insert([
//                                         'document_image' => 'transport_document/'.$imageName,
//                                         'document_id' => !empty($request->document_id[$file_key])?$request->document_id[$file_key]:'0',
//                                         'transport_id' => $request->dms_transport_id,
//                                         'company_id' => $company_id,
//                                         'date' => date('Y-m-d'),
//                                         'time' => date('H:i:s'),
//                                         'server_date_time' => date('Y-m-d H:i:s'),
